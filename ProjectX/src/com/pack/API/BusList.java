...
public class BusList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			BusList.LineTypeNameLists = doc.getElementsByTagName("LineTypeName");
			BusList.NoLists = doc.getElementsByTagName("Name");
			BusList.TowardsLists = doc.getElementsByTagName("Towards");
			BusList.JourneyDateTimeLists = doc.getElementsByTagName("JourneyDateTime");
			...
			out.print("<ul>");
				...
				for (int index = 0; index < LineTypeNameLists.getLength(); index++) {
					StringBuffer theString = new StringBuffer(BusList.JourneyDateTimeLists.item(index).getTextContent());
					...
					if (currentTime.isBefore(endTime)) {						
						...
						if (encoding(BusList.LineTypeNameLists.item(index).getTextContent()).matches("SkåneExpressen")) {
							out.print("<li>" + encoding(BusList.NoLists.item(index).getTextContent())
								+ " mot " + encoding(BusList.TowardsLists.item(index).getTextContent().replaceAll("SkÃ¥neExpressen ", "")) + "<br> Avgångstid: "
								+ departureTime + "</li><br><br>");
						} else if (!encoding(BusList.LineTypeNameLists.item(index).getTextContent()).matches("Pågatågen|Öresundståg|Flygbuss")) {
								out.print("<li>" + encoding(BusList.LineTypeNameLists.item(index).getTextContent()) + " " + encoding(BusList.NoLists.item(index).getTextContent())
								+ " mot " + encoding(BusList.TowardsLists.item(index).getTextContent()) + "<br> Avgångstid: "
								+ departureTime + "</li><br><br>");
						}
					}
				}
